...
void Controller::DrawGame()//绘制游戏界面
{
    ...
    SetCursorPosition(31, 7); /*计分板*/
    std::cout << "玩家得分：" ;
    SetCursorPosition(37, 8);
    std::cout << "     0" ;
	SetCursorPosition(31, 9); /*计分板*/
	std::cout << "人机蛇得分：";
...
int Controller::PlayGame()//游戏二级循环
{
    ...
    while (csnake->OverEdge() && csnake->HitItself()) //判断是否撞墙或撞到自身，即是否还有生命
    {
        ...
        {
            msnake->mMove(*cfood);
            UpdateaiScore(1);//更新分数，1为分数权重
            RewriteaiScore();//重新绘制分数
            cfood->DrawFood(*csnake);//绘制新食物
        }
        else
        {
		...
		if (csnake->GetSpecialFood(*cfood)) //吃到特殊食物
		{
            PlaySound(MAKEINTRESOURCE(IDR_WAVE2), NULL, SND_RESOURCE | SND_ASYNC);
			csnake->Move();//蛇增长
			UpdateScore(2);//更新分数，2为分数权重
			RewriteScore();//重新绘制分数
//			cfood->DrawFood(*csnake);
//			cfood->DrawFood(*csnake);//绘制两个新食物
            msnake->special(32);
		}
        ...
        {
            msnake->mMove(*cfood);
            UpdateaiScore(cfood->GetProgressBar() / 5);//分数根据限时食物进度条确定
            RewriteaiScore();
        }
...
void Controller::UpdateaiScore(const int& tmp)//更新AI蛇分数
{
	aiscore += key * 10 * tmp;//所得分数根据游戏难度及传人的参数tmp确定
}
...
void Controller::RewriteaiScore()//重绘AI蛇分数
{
	/*为保持分数尾部对齐，将最大分数设置为6位，计算当前分数位数，将剩余位数用空格补全，再输出分数*/
	SetCursorPosition(37, 10);
	SetColor(11);
